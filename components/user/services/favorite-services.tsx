...
export default function FavoriteServices() {
  ...
  useEffect(() => {
    const fetchFavoriteServices = async () => {
      ...
      try {
        if (!user?.id) {
          toast.error('You need to be logged in to view favorite services');
          setLoading(false);
          return;
        }
        ...
        toast.error('Error fetching favorite services. Please try again later.');
        setServices([]);
        setGroupedServices({});
        setTotalPages(1);
      } finally {
  ...
  const toggleFavorite = async (serviceId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(data.message || 'Service removed from favorites');
      } else {
