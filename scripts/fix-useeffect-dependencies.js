...
// Files that were updated by the previous script
const filesToUpdate = [
...
function fixUseEffectDependencies(filePath) {
  ...
  // Fix useEffect dependency arrays that are missing appName
  const useEffectRegex = /(useEffect\(\(\) => {[\s\S]*?setPageTitle\([^)]+, appName\);[\s\S]*?}, \[)\]/g;
  
  const newContent = content.replace(useEffectRegex, (match, prefix) => {
  ...
  if (modified) {
    ...
    console.log(`Fixed useEffect dependencies: ${filePath}`);
  } else {
...
// Fix all files
console.log('Starting useEffect dependency fix...');
filesToUpdate.forEach(fixUseEffectDependencies);
console.log('useEffect dependency fix completed!');