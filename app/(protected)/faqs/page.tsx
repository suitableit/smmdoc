...
const FAQPage = () => {
  ...
  const categoryNames: Record<string, { name: string; icon: React.ReactNode }> = {
    'general': { name: 'General Questions', icon: <FaQuestionCircle className="w-5 h-5" /> },
    'orders': { name: 'Orders', icon: <FaShoppingCart className="w-5 h-5" /> },
    'payment': { name: 'Payment & Billing', icon: <FaCreditCard className="w-5 h-5" /> },
    'api': { name: 'API & Integration', icon: <FaCode className="w-5 h-5" /> },
    'reseller': { name: 'Reseller Services', icon: <FaUserTie className="w-5 h-5" /> },
        ...
        <div className="card card-padding">
          ...
          {Object.keys(groupedFAQs).length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <div className="text-lg">No questions found matching your search.</div>
