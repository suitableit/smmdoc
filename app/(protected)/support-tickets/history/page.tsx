...
type Ticket = {
  id: string;
  subject: string;
  status: 'open' | 'answered' | 'customer_reply' | 'on_hold' | 'in_progress' | 'closed';
  createdAt: string;
...
export default function TicketsHistory() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchInput.trim()) {
        ...
        const results = dummyTickets.filter(ticket => 
  ...
  const statusFilters = [
    ...
    { key: 'open', label: 'Open', icon: FaClock, color: 'bg-gray-600 hover:bg-gray-700' },
    { key: 'answered', label: 'Answered', icon: FaCheckCircle, color: 'bg-gray-600 hover:bg-gray-700' },
    { key: 'customer_reply', label: 'Customer Reply', icon: FaTicketAlt, color: 'bg-gray-600 hover:bg-gray-700' },
    { key: 'on_hold', label: 'On Hold', icon: FaSpinner, color: 'bg-gray-600 hover:bg-gray-700' },
    { key: 'in_progress', label: 'In Progress', icon: FaSpinner, color: 'bg-gray-600 hover:bg-gray-700' },
    { key: 'closed', label: 'Closed', icon: FaBan, color: 'bg-gray-600 hover:bg-gray-700' }
  ];
  ...
  const getPriorityBadge = (priority?: string) => {
    ...
    switch (priority) {
      case 'high':
        return <span className={`${baseClasses} bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-200`}>High</span>;
      case 'medium':
        return <span className={`${baseClasses} bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-200`}>Medium</span>;
      case 'low':
        return <span className={`${baseClasses} bg-blue-100 text-blue-800 dark:bg-blue-900/20 dark:text-blue-200`}>Low</span>;
      default:
        return null;
    }
  };
                  ...
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-gray-500">
                      <div className="flex flex-col items-center">
                        ...
                        <div className="text-lg font-medium">No {activeTab === 'all' ? '' : activeTab === 'human' ? 'human support' : 'AI'} tickets found</div>
