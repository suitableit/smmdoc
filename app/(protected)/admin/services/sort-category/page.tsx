...
export default function SortByCategoryPage() {
  ...
  const breadcrumbItems = [
    ...
    { title: 'Sort by Category', link: '/admin/services/sort-category' },
  ];
  ...
  const fetchCategoriesWithServices = async () => {
    try {
      ...
      if (categoriesResult.success && servicesResult.success) {
        ...
        const categoriesWithServices = categoriesResult.data.map(
          (category: any) => ({
            ...category,
  ...
  const activeServices = categories.reduce(
    (sum, cat) =>
      sum + cat.services.filter((s) => s.status === 'active').length,
    0
  );
    ...
    <div className="h-full space-y-6 p-6 bg-gradient-to-br from-gray-50 to-purple-50 min-h-screen">
      ...
      <div className="space-y-6">
        {filteredCategories.map((category) => (
          <Card
            key={category.id}
            className="relative overflow-hidden border-0 bg-white shadow-lg"
          >
                  ...
                  {category.services.map((service, index) => (
                    ...
                    >
                      ...
                      >
                        <div
                          className={
                            viewMode === 'grid' ? 'space-y-2' : 'flex-1'
                          }
                        >
                          <div className="flex items-center gap-2">
