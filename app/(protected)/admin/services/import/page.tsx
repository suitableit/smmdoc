...
const ImportServicesPage = () => {
  ...
  const dummyProviders: Provider[] = [
    { 
      ...
      status: 'active',
      description: 'Premium social media marketing services provider with high-quality followers, likes, and engagement.'
    },
    { 
      id: 'provider_002', 
      ...
      status: 'active',
      description: 'Fast delivery SMM services with 24/7 support and guaranteed results for all major platforms.'
    },
    { 
      id: 'provider_003', 
      name: 'SMM Heaven API', 
      ...
      status: 'inactive',
      description: 'Cost-effective social media marketing solutions with bulk discounts and API integration.'
    },
    { 
      id: 'provider_004', 
      ...
      status: 'active',
      description: 'Professional grade SMM services with advanced targeting and analytics tracking.'
    }
  ];

  // Dummy data for API categories
  const dummyApiCategories: ApiCategory[] = [
    { id: 'cat_001', name: 'Instagram Services', servicesCount: 45, selected: false },
    { id: 'cat_002', name: 'Facebook Services', servicesCount: 32, selected: false },
    { id: 'cat_003', name: 'YouTube Services', servicesCount: 28, selected: false },
    { id: 'cat_004', name: 'TikTok Services', servicesCount: 35, selected: false },
    { id: 'cat_005', name: 'Twitter Services', servicesCount: 22, selected: false },
    { id: 'cat_006', name: 'LinkedIn Services', servicesCount: 18, selected: false },
    { id: 'cat_007', name: 'Pinterest Services', servicesCount: 15, selected: false },
    { id: 'cat_008', name: 'Snapchat Services', servicesCount: 12, selected: false }
  ];

  // Dummy data for services
  const dummyServices: Service[] = [
    { id: 'srv_001', name: 'Instagram Followers - High Quality', category: 'Instagram Services', min: 100, max: 100000, rate: 2.50, description: 'Premium Instagram followers with real accounts and gradual delivery', type: 'followers' },
    { id: 'srv_002', name: 'Instagram Likes - Instant', category: 'Instagram Services', min: 50, max: 50000, rate: 1.20, description: 'Fast Instagram likes delivery within minutes', type: 'likes' },
    { id: 'srv_003', name: 'Instagram Views - Story', category: 'Instagram Services', min: 100, max: 25000, rate: 0.80, description: 'Instagram story views from real active users', type: 'views' },
    { id: 'srv_004', name: 'Facebook Page Likes', category: 'Facebook Services', min: 100, max: 20000, rate: 3.00, description: 'Organic Facebook page likes with lifetime guarantee', type: 'likes' },
    { id: 'srv_005', name: 'Facebook Post Likes', category: 'Facebook Services', min: 25, max: 10000, rate: 1.50, description: 'Fast Facebook post likes from real users', type: 'likes' },
    { id: 'srv_006', name: 'YouTube Views - Real', category: 'YouTube Services', min: 1000, max: 1000000, rate: 4.00, description: 'High retention YouTube views from real viewers', type: 'views' },
    { id: 'srv_007', name: 'YouTube Subscribers', category: 'YouTube Services', min: 100, max: 50000, rate: 8.00, description: 'Real YouTube subscribers with profile pictures', type: 'followers' },
    { id: 'srv_008', name: 'TikTok Followers', category: 'TikTok Services', min: 100, max: 30000, rate: 2.20, description: 'TikTok followers from active profiles', type: 'followers' },
    { id: 'srv_009', name: 'TikTok Likes', category: 'TikTok Services', min: 50, max: 25000, rate: 1.10, description: 'TikTok video likes with fast delivery', type: 'likes' },
    { id: 'srv_010', name: 'Twitter Followers', category: 'Twitter Services', min: 100, max: 15000, rate: 3.50, description: 'Real Twitter followers with complete profiles', type: 'followers' },
    { id: 'srv_011', name: 'Twitter Retweets', category: 'Twitter Services', min: 25, max: 5000, rate: 2.00, description: 'Twitter retweets from genuine accounts', type: 'shares' },
    { id: 'srv_012', name: 'LinkedIn Connections', category: 'LinkedIn Services', min: 50, max: 2000, rate: 5.00, description: 'Professional LinkedIn connections', type: 'followers' }
  ];
  ...
  const handleCategorySelect = (categoryId: string) => {
    setApiCategories(prev => prev.map(cat => 
  ...
  const handleSelectAllCategories = () => {
    ...
    setApiCategories(prev => prev.map(cat => ({ ...cat, selected: !allSelected })));
  };
  ...
  const loadCategories = async () => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
      showToast(`Loaded ${dummyApiCategories.length} categories from ${getProviderName(selectedProvider)}`, 'success');
    }, 2000);
  };
  ...
  const handleSaveServices = async () => {
    try {
      ...
      showToast(`Successfully imported ${totalServices} services${modifiedServices > 0 ? ` (${modifiedServices} customized)` : ''}`, 'success');
      ...
      setIsLoading(false);
      showToast(`Error importing services: ${error.message || 'Unknown error'}`, 'error');
    }
  };
                ...
                <div>
                  ...
                  <div className="max-w-md">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          value={profitPercent}
                          onChange={(e) => setProfitPercent(Math.max(0, Math.min(100, parseInt(e.target.value) || 0)))}
                    ...
                    <p className="text-xs mt-2" style={{ color: 'var(--text-muted)' }}>
                      This percentage will be added to all imported service prices as your profit margin.
                    </p>
                  </div>
                </div>
                ...
                {selectedProvider && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                    ...
                    <div className="text-sm text-blue-700 space-y-1">
                      <p><strong>Provider:</strong> {getProviderName(selectedProvider)}</p>
                      <p><strong>Profit Margin:</strong> {profitPercent}%</p>
                      <p><strong>Example:</strong> If provider price is $1.00, your price will be ${(1 * (1 + profitPercent / 100)).toFixed(2)}</p>
                    </div>
                  </div>
                )}
              </div>
                  ...
                  <div className="flex items-center justify-center py-20">
                    <div className="text-center flex flex-col items-center">
                      ...
                      <div className="text-base font-medium">
                        Loading categories from {getProviderName(selectedProvider)}...
                      </div>
                    </div>
                  </div>
                ) : (
                    ...
                    {apiCategories.some(cat => cat.selected) && (
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                        ...
                        <div className="text-sm text-green-700 space-y-1">
                          <p>
                            <strong>Selected Categories:</strong> {apiCategories.filter(cat => cat.selected).length}
                          </p>
                          <p>
                            <strong>Total Services:</strong> {apiCategories.filter(cat => cat.selected).reduce((sum, cat) => sum + cat.servicesCount, 0).toLocaleString()}
                          </p>
                        </div>
                      </div>
                    )}
            ...
            {currentStep === 3 && (
              <div className="space-y-6">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-semibold text-blue-800">Services Ready for Import</h4>
                      <p className="text-sm text-blue-700">
                        {services.length} services loaded with {profitPercent}% profit margin applied
                      </p>
                    </div>
                    <div className="text-right">
                      <div className="text-sm text-blue-700">
                        <strong>Provider:</strong> {getProviderName(selectedProvider)}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {Object.keys(groupedServices).length === 0 ? (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                      {searchTerm ? 'No services match your search criteria.' : 'No services available for selected categories.'}
                    </p>
                  </div>
                ) : (
                  <>
