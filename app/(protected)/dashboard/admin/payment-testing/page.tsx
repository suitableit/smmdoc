...
import { CheckCircle, Clock, XCircle, Send, TestTube, Smartphone, Mail } from 'lucide-react';
...
export default function PaymentTestingPage() {
  ...
  const breadcrumbItems = [
    ...
    { title: 'Payment Testing', link: '/dashboard/admin/payment-testing' },
  ];
  ...
  const runPaymentTest = async () => {
    ...
    try {
      ...
      const createResponse = await fetch('/api/admin/testing/create-test-payment', {
        method: 'POST',
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setTestData({...testData, invoice_id: e.target.value})}
            ...
            <div>
              ...
              <Select value={testData.response_type} onValueChange={(value) => setTestData({...testData, response_type: value})}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="completed">Completed (Auto-approve)</SelectItem>
                  <SelectItem value="pending">Pending (Manual review)</SelectItem>
            ...
            <div className="flex gap-2">
              <Button onClick={generateTestData} variant="outline" className="flex-1">
                Generate Test Data
              </Button>
              <Button onClick={runPaymentTest} disabled={isLoading} className="flex-1">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={testSMSNotification} variant="outline" size="sm">
                ...
                {testResults.map((result) => (
                  <div key={result.id} className="border rounded-lg p-4 space-y-2">
                    <div className="flex justify-between items-start">
                      ...
                      {result.response.status && getStatusBadge(result.response.status)}
                    </div>
                    
                    <div className="text-sm">
                      <strong>Input:</strong> {result.input.response_type} - ৳{result.input.amount}
                    </div>
                    
                    <div className="text-sm">
                      <strong>Response:</strong> {result.response.message || result.response.error}
                    </div>
                    
                    {result.response.payment && (
                      <div className="text-xs text-gray-600">
                        Invoice: {result.response.payment.invoice_id} | 
                        Status: {result.response.payment.status}
                      </div>
                    )}
                  </div>
                ))}
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="border rounded-lg p-4">
              <h4 className="font-medium text-green-700 mb-2">✅ Success Flow</h4>
              ...
              <Button 
                ...
                onClick={() => {
                  generateTestData();
                  setTestData(prev => ({...prev, response_type: 'completed'}));
                }}
              >
                Setup Success Test
              </Button>
            ...
            <div className="border rounded-lg p-4">
              <h4 className="font-medium text-yellow-700 mb-2">⏳ Pending Flow</h4>
              <p className="text-sm text-gray-600 mb-3">
              ...
              <Button 
                ...
                onClick={() => {
                  generateTestData();
                  setTestData(prev => ({...prev, response_type: 'pending'}));
                }}
              >
                Setup Pending Test
              </Button>
            ...
            <div className="border rounded-lg p-4">
              ...
              <Button 
                ...
                onClick={() => {
                  generateTestData();
                  setTestData(prev => ({...prev, response_type: 'cancelled'}));
                }}
              >
                Setup Failed Test
              </Button>
