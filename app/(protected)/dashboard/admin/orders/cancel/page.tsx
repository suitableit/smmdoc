...
export default function CancelOrdersPage() {
  ...
  const breadcrumbItems = [
    ...
    { title: 'Cancel Orders', link: '/dashboard/admin/orders/cancel' },
  ];
  ...
  const fetchCancellableOrders = async () => {
    try {
      ...
      if (result.success) {
        ...
        const cancellableOrders = result.data.filter((order: Order) => 
    ...
    try {
      ...
      const response = await fetch(`/api/admin/orders/${selectedOrder.id}/cancel`, {
        ...
        body: JSON.stringify({
          ...
          customRefundAmount: cancelForm.refundType === 'custom' ? parseFloat(cancelForm.customRefundAmount) : undefined
        }),
      });
      ...
      if (result.success) {
        ...
        setCancelInfo(null);
        setCancelForm({ reason: '', refundType: 'full', customRefundAmount: '' });
      ...
      <div className="h-full space-y-6 p-6 bg-gradient-to-br from-gray-50 to-red-50 min-h-screen">
        ...
        <div className="flex items-center justify-center py-20">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-medium">Loading cancellable orders...</span>
          </div>
        </div>
      </div>
    );
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="relative overflow-hidden group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border-0 bg-gradient-to-br from-red-50 to-rose-100">
          ...
          <CardContent className="p-6 flex items-center justify-between relative z-10">
            <div>
              ...
              <p className="text-sm font-medium text-gray-600">Cancellable Orders</p>
            </div>
        ...
        <Card className="relative overflow-hidden group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border-0 bg-gradient-to-br from-yellow-50 to-amber-100">
          ...
          <CardContent className="p-6 flex items-center justify-between relative z-10">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-3xl font-bold text-gray-800">{filteredOrders.filter(o => o.status === 'pending').length}</p>
        ...
        <Card className="relative overflow-hidden group hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 border-0 bg-gradient-to-br from-blue-50 to-indigo-100">
          ...
          <CardContent className="p-6 flex items-center justify-between relative z-10">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <p className="text-3xl font-bold text-gray-800">{filteredOrders.filter(o => ['processing', 'in_progress'].includes(o.status)).length}</p>
        ...
        <CardContent>
          {filteredOrders.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-800 mb-2">No cancellable orders found</h3>
              <p className="text-gray-500 text-sm">No orders are currently available for cancellation or no orders match your filters.</p>
            </div>
          ) : (
      ...
      <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Cancel order #{selectedOrder?.id.slice(-8)}. This action will stop the order processing and may issue a refund.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 bg-red-50 rounded-lg border border-red-200">
                <div>
                  ...
                  <div className="font-semibold text-red-600 capitalize">{cancelInfo.order.status}</div>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">Total Quantity</Label>
                  <div className="font-semibold text-gray-900">{cancelInfo.order.totalQuantity.toLocaleString()}</div>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">Delivered</Label>
                  <div className="font-semibold text-green-600">{cancelInfo.order.deliveredQuantity.toLocaleString()}</div>
                </div>
                <div>
                  <Label className="text-sm font-medium text-gray-700">Remaining</Label>
                  <div className="font-semibold text-orange-600">{cancelInfo.order.remainingQuantity.toLocaleString()}</div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="font-semibold text-green-600">
                        ${cancelInfo.refund.amount.toFixed(2)} {cancelInfo.refund.currency}
                      </div>
                    </div>
                    <div>
                      <Label className="text-sm font-medium text-gray-700">User Balance After</Label>
                      <div className="font-semibold text-green-600">
                        ${(cancelInfo.user.balance + cancelInfo.refund.amount).toFixed(2)} {cancelInfo.user.currency}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                    <input
                      ...
                      onChange={(e) => setCancelForm(prev => ({ ...prev, refundType: e.target.value }))}
                      className="text-red-600"
                    />
                    <label htmlFor="full-refund" className="flex-1 cursor-pointer">
                      <div className="font-medium">Full Refund</div>
                      <div className="text-sm text-gray-500">
                        Refund the entire order amount: ${cancelInfo.order.totalCost.toFixed(2)}
                      </div>
                    </label>
                  </div>
                  ...
                  <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    <label htmlFor="partial-refund" className="flex-1 cursor-pointer">
                      <div className="font-medium">Partial Refund (Remaining Only)</div>
                      <div className="text-sm text-gray-500">
                        Refund only for undelivered quantity: ${cancelInfo.refund.amount.toFixed(2)}
                      </div>
                    </label>
                  </div>
                  ...
                  <div className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50 transition-colors">
                    <input
                      ...
                      className="text-red-600"
                    />
                    <label htmlFor="custom-refund" className="flex-1 cursor-pointer">
                ...
                {cancelForm.refundType === 'custom' && (
                  <div className="mt-3">
                    ...
                    <Input
                      ...
                      onChange={(e) => setCancelForm(prev => ({ ...prev, customRefundAmount: e.target.value }))}
                      className="mt-1"
                    />
                  </div>
                )}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700">
                      This action cannot be undone. The order will be permanently cancelled and any refund will be processed immediately.
                    </p>
                  </div>
                </div>
              </div>
