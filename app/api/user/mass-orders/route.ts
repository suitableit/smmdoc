...
export async function POST(request: Request) {
  try {
    ...
    if (!Array.isArray(orders) || orders.length === 0) {
      return NextResponse.json(
        { success: false, message: 'Orders must be a non-empty array', data: null },
        { status: 400 }
      );
    }
    
    // Limit mass orders to prevent abuse
    if (orders.length > 100) {
      return NextResponse.json(
        { success: false, message: 'Maximum 100 orders allowed per mass order request', data: null },
        { status: 400 }
      );
    }
    ...
    for (let i = 0; i < orders.length; i++) {
      ...
      try {
        ...
        if (!categoryId || !serviceId || !link || !qty) {
          validationErrors.push(`Order ${i + 1}: Missing required fields (categoryId, serviceId, link, qty)`);
          continue;
        }
        ...
        if (!service) {
          validationErrors.push(`Order ${i + 1}: Service not found (${serviceId})`);
          continue;
        }
        
        if (service.status !== 'active') {
          validationErrors.push(`Order ${i + 1}: Service '${service.name}' is not active`);
          continue;
        }
        ...
        if (isNaN(quantity) || quantity < service.min_order || quantity > service.max_order) {
          validationErrors.push(`Order ${i + 1}: Quantity for '${service.name}' must be between ${service.min_order} and ${service.max_order}`);
          continue;
        }
        ...
        validatedOrders.push({
          ...
          remains: quantity,
          startCount: 0,
          isMassOrder: true, // Mark as mass order
          batchId: batchId || `MO-${Date.now()}-${session.user.id.slice(-4)}`, // Mass order batch ID
          service: {
            name: service.name,
            rate: service.rate
          }
        ...
        validationErrors.push(`Order ${i + 1}: Validation error - ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    if (user.balance < totalCost) {
      return NextResponse.json(
        { 
          success: false, 
          message: `Insufficient balance. Required: ${totalCost.toFixed(2)} ${user.currency}, Available: ${user.balance.toFixed(2)} ${user.currency}`, 
          data: null,
          summary: {
            totalCost,
    ...
    // Log the mass order creation
    console.log(`User ${session.user.email} created ${result.length} mass orders`, {
      userId: session.user.id,
...
// GET /api/user/mass-orders - Get mass order templates or history
export async function GET(request: Request) {
  try {
    ...
    if (type === 'recent') {
      // Get recent orders grouped by creation time (potential mass orders)
    ...
    console.error('Error fetching mass order data:', error);
    return NextResponse.json(
      { 
        success: false, 
        message: 'Error fetching mass order data', 
        error: error instanceof Error ? error.message : 'Unknown error' 
      },
      { status: 500 }
    );
